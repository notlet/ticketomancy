...
module.exports = {
    ...
    exec: async (input, message) => {
        ...
        if (!config.categories[ticket.type].team.map(r => message.member.roles.cache.has(r)).includes(true)) throw new PermissionsError('You can\'t unclaim this ticket!');
        ...
        for (const t of config.categories[ticket.type].team) await message.channel.permissionOverwrites.edit(t, { SendMessages: true });
